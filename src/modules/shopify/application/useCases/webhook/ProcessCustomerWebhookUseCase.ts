...
export class ProcessCustomerWebhookUseCase extends BaseService {
    ...
    async execute(input: {
        ...
        const entityType = ShopifyEntityType.fromString("customer");
        ...
        const customer = await this.shopifyClient.fetchCustomerById({
            credentials,
            externalId: input.entityId,
        });

        if (!customer) {
            logger.warn("webhook.customer_not_found_in_shopify", {
                tenantId: input.tenantId,
                externalId: input.entityId,
            });
            return;
        ...
        const hash = this.changeDetectionService.computeHash(customer);
